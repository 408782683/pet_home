...
public class UserMapper {
    ...
    public void registerUser(User user) throws SQLException {
        String sql = "insert into user(username,password,role,nickname,avatar,phone,email,status,create_time,money) values(?,?,?,?,?,?,?,?,now(),?)";
        try(Connection conn = JdbcUtil.getConnection();
            PreparedStatement ps = conn.prepareStatement(sql)){
    ...
    public User loginUser(String username,String password,String role){
        ...
        try(Connection connection = JdbcUtil.getConnection();
            ...
            while(rs.next()){
                ...
                if(BCrypt.checkpw(password,bcPwd) && role.equals(rs.getString("role")) && status.equals("正常")){
    ...
    public int updateProfile(User user) throws SQLException {
        String sql = "update user set nickname=?,avatar=?,phone=?,email=? where id=?";
        try(Connection conn = JdbcUtil.getConnection();
            PreparedStatement ps = conn.prepareStatement(sql)) {
