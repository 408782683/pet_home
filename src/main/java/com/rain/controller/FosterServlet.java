    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            if ("/listByPage".equals(path)) {
                JSONObject result = service.pageQuery("foster_post", body, body.getIntValue("page", 1), body.getIntValue("size", 12), "id desc");
                JSONObject data = new JSONObject();
                data.put("fosterList", result.getJSONArray("list"));
                data.put("total", result.getInteger("total"));
                writeJson(resp, success(data));
            } else if ("/add".equals(path)) {
                service.savePost("foster_post", body, false, body.getInteger("userId"));
                writeJson(resp, success("发布成功"));
            } else if ("/update".equals(path)) {
                int n = service.savePost("foster_post", body, true, body.getInteger("currentUserId"));
                writeJson(resp, n > 0 ? success("更新成功") : error("未找到可更新帖子"));
            } else if ("/detail".equals(path)) {
                writeJson(resp, success(service.getById("foster_post", req.getParameter("id") == null ? body.getLongValue("id") : Long.parseLong(req.getParameter("id")))));
            } else {
